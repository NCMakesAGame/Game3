...
public class Camera {
    ...
    * @param map2 the TiledMap used for the current scene
    */
   public Camera(GameContainer gc, BlockMap map2) {
      this.map = map2;
      
      this.numTilesX = BlockMap.map.getWidth();
      this.numTilesY = BlockMap.map.getHeight();
      
      this.tileWidth = 32;//tile width
      this.tileHeight = 32; //tile height
   ...
   public void drawMap(int offsetX, int offsetY) {
    ...
    //   BlockMap.map.render(0,0,0,0,100,100,1,false); render(   
       BlockMap.map.render(tileOffsetX + offsetX, 
             tileOffsetY + offsetY, 
             tileIndexX,  
             tileIndexY,
